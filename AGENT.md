...
### 2026-06-17 Preview 경량화 및 MHR 조사 메모
현재 사용 버전을 가볍게 만들기 위해 3D preview 쪽에 최소 변경을 적용했습니다.

- `메시 보기`는 기본적으로 꺼져 있고, 켰을 때도 face를 `mesh_preview_face_stride = 8` 간격으로 일부만 그립니다.
- 회전 슬라이더를 드래그하는 동안에는 mesh 렌더링을 생략하고 skeleton만 갱신합니다. 마우스를 놓으면 mesh가 다시 갱신됩니다.
- 중복으로 pack되던 `손 모드` 체크박스 한 번을 제거했습니다.

MHR -> SMPL 변환 조사 결과:

- `Conversion.convert_mhr2smpl(...)` 공개 API에는 반복 횟수/품질 preset 인자가 없습니다.
- 내부적으로 `PyTorchSMPLFitting.fit()` -> `_optimize_smpl(...)` 경로를 타며, 기본 반복 수는 `((40, 80, 40), 300)`입니다.
- 이 반복 수가 20초대 변환 시간의 주된 원인으로 보입니다.
- 빠른 모드를 만들려면 `MHR-main/tools/mhr_smpl_conversion/pytorch_fitting.py`와 `conversion.py`에 선택적 `iterations` 인자를 전달하도록 손봐야 합니다. 외부 라이브러리 성격의 코드라 현재 안정 버전에는 바로 적용하지 않았습니다.
